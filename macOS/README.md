...
| P-9 | The mark is placed by **measuring its circular outline** — not by fitting the artwork's square, and not by fitting the bounding box of everything drawn. The circle is what a reader sees as the mark; the leaves are an accent that sticks out of it at one corner, so centring on the ink box leans the round part up and to the left. Measured: the ink box's centre is (0.489, 0.484) of the artwork against the circle's (0.539, 0.520), a gap of about a twentieth of the icon. `anchorBounds(of:)` renders the logo once and takes the bounds of ink that is dark **and** near-neutral, which comes back 0.713 square on this mark. |
...
| P-11 | The circle is **0.66 of the plate and centred on it to the pixel**, read back off the generated `.icns` rather than asserted. The artwork around it is deliberately *not* centred — measured margins inside the plate are 33pt left against 71pt right, 44pt top against 71pt bottom — because the leaves reach further up and left than down and right. The icon reads straight anyway, which is the whole claim: the round part is where the eye expects it. |
...
| Change | Summary |
| --- | --- |
| The kirupa mark | The application icon is the kirupa logo on white, in the Dock, in Finder, on the landing screen and on the iOS build — drawn from `Packaging/Logo.svg` and placed by measuring its circular outline, so the circle is the optical centre and the leaves are free to hang off one corner. See [P-7 to P-11](#3-platform-and-technical-requirements). |
