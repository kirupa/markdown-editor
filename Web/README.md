...
| WR-32a | **An Apple Developer Program signing identity is required before FirebaseAuth can sign in on macOS at all**, and this cannot be worked around from this repository. FirebaseAuth persists its session to the macOS data-protection keychain, which requires the `keychain-access-groups` entitlement; that entitlement is *restricted*, so it must be authorized by a provisioning profile, which requires a Team ID. Measured, not inferred, by `Shared/Firebase/Scripts/check-keychain.sh`: the same `signInAnonymously()` fails with `SecItemAdd (-34018)` as a bare executable **and** from inside an ad-hoc-signed `.app` — the exact form `macOS/Scripts/build-app.sh` produces — and a bundle that claims the entitlement without a profile is SIGKILLed at exec (exit 137). All three, every run. This affects the **native macOS** build only: the web build does not use the Apple SDK, and the Firestore adapter underneath is unaffected and is verified ([macOS README §16.6](../macOS/README.md#166-the-native-firestore-adapter-is-verified-against-a-real-firestore)). |
