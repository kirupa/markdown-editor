...
| I-172 | Leaving the writing **hands the pointer back**. The I-beam is set imperatively rather than claimed with a rect, so AppKit does not know to undo it — measured, `super.mouseExited` leaves it exactly where it was, and without this the text cursor simply walks out of the document with the pointer and parks over the margin. This is half the bug arriving by a second route, and the first fix did not cover it. |
| I-173 | The bar's icons take the **pointing hand** on hover, with `set` rather than `push`. `onHover` is not guaranteed to be balanced — a window losing key under the pointer drops the exit — and an unbalanced push leaves that shape over the whole application until it is relaunched, which is a far worse failure than a stale shape the next movement corrects. |
...
| A pointer that knows where it is | The I-beam is confined to the writing at last. It was leaking over the toolbar, the margins and the critique rail because the text view answers mouse moves for the *whole window* — it is the first responder, and the window forwards them — and its "which shape" function has no case for "not over the writing". Four earlier rounds on cursor rects, tracking areas and hover were aimed at the wrong cause, and two of them were reported as fixed. See [I-170 to I-176](#10a-ai-assisted-critique). |
...
| Pointer shapes verified on every run | `make check-image-layout` asserts what `pointerCursor` answers at each place that matters, *and* drives the real `mouseMoved` entry point to read the cursor AppKit ends up holding — including at points outside the view, which is where the I-beam was leaking from. Whether AppKit then *shows* that shape on a screen is still a separate question, and one this app has got wrong repeatedly while in-process checks passed — see I-57 and I-174. `make check-image-cursors` settles it against real screen pixels, but it drives the real mouse and needs an unlocked screen, so it is opt-in and not part of any suite. **Its toolbar and margin probes have not yet been run**: the screen was locked when they were written, and they are unverified until somebody runs them. |
