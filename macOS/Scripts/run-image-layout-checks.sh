#!/bin/bash
# Builds and runs the image layout checks.
#
...
# Scripts/check-image-layout.swift, linking against the object files SPM has
...
swiftc -parse-as-library -o "$OUT/check-image-layout" \
    $SOURCES Scripts/check-image-layout.swift $OBJECTS \
    -I "$BIN_DIR/Modules"

"$OUT/check-image-layout"
