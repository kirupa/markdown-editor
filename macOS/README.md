...
| I-174 | This was reported fixed **twice** on reasoning alone, and was wrong both times. The in-process checks are the ones that finally located it, by driving the real `mouseMoved` entry point with a point outside the view and reading the cursor AppKit ends up holding — see `make check-image-layout`. Synthetic pointer *warps* move the cursor without delivering events to the application, so anything hover-driven reads as working when it is not. |
