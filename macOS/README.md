...
| P-9 | The mark is **sized by its circular outline** — not by the artwork's square, and not by the bounding box of everything drawn. The circle is what a reader sees as the mark; the leaves are an accent that sticks out of it at one corner, so fitting the ink box shrinks the round part and leans it. Measured: the ink box's centre is (0.489, 0.484) of the artwork against the circle's (0.540, 0.520), about a twentieth of the icon apart. `measureMark(in:)` renders the logo once and takes the bounds of ink that is dark **and** near-neutral, which comes back 0.713 square on this mark. |
...
| P-11 | The circle is **0.66 of the plate**, and the icon is **centred by the ink that circle encloses** rather than by the circle's geometry. Centring on the geometry measures perfect and still looks wrong: the ring is not closed — the leaves cross it and leave a 40° gap from 9 to 11 o'clock, confirmed by walking the fitted circle degree by degree — so the dark shape is a C, and a C weighs more on the side away from its opening. The eye reads the weight. |
| P-12 | Correcting all the way to the **dark ring's own centre of mass overshoots**, and this was settled by rendering the options side by side rather than by argument. It shifts the mark until the leaves crowd the plate's corner and an visibly empty quarter opens opposite — measured margins of 42pt left against 166pt, 63pt top against 167pt. Balancing the ink *inside* the circle instead moves it about a quarter as far: margins 62pt left against 147pt, 77pt top against 154pt, from 66/143 and 88/143 before. The leaves stay an accent that overhangs, because an accent that drags the composition is not an accent. |
| P-13 | Nothing is clipped, and that is measured rather than eyeballed: the nearest ink sits **46.8pt inside the plate's rounded edge**, 5.5% of the plate, with zero pixels outside it. The check is a signed distance to the rounded rectangle, so the corners are tested as curves rather than as a box. |
...
| Change | Summary |
| --- | --- |
| The kirupa mark | The application icon is the kirupa logo on white, in the Dock, in Finder, on the landing screen and on the iOS build — drawn from `Packaging/Logo.svg`, sized by its circular outline and balanced on the ink that circle encloses, so the round part sits where the eye expects it and the leaves hang off one corner. See [P-7 to P-13](#3-platform-and-technical-requirements). |
