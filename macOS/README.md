...
516 tests across 36 suites, in the shared package:
...
| Critique what changed, not the whole draft again | An edited draft offers "Critique the changes" first: the model reads everything for context but writes findings only about the paragraphs that moved, and the notes about the rest are kept rather than replaced. "Everything" is one click away. Also splits the rail into two faces — the reviewer's hand for their notes, the system sans for the app's own words. See [I-181 to I-192](#10a-ai-assisted-critique). |
