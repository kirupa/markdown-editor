...
| P-9 | The mark is placed by **measuring its ink**, not by fitting the artwork square. The logo is neither centred in its own viewBox nor filling it, so drawn as a plain square it came out at 0.60 of the plate and 3.5pt off centre. `inkBounds(of:)` renders it once, finds the bounds of everything that is neither transparent nor white, and the icon is sized and centred on that — measured back at exactly 0.80 of the plate with zero offset. White is deliberately not ink: the logo carries a white halo and a white disc that are invisible on a white plate, and counting them would reserve room for something nobody can see. |
...
| Change | Summary |
| --- | --- |
| The kirupa mark | The application icon is the kirupa logo on white, in the Dock, in Finder, on the landing screen and on the iOS build — drawn from `Packaging/Logo.svg` and placed by measuring its ink rather than by fitting its square. See [P-7 to P-9](#3-platform-and-technical-requirements). |
