...
| I-115 | **The page is a sheet of paper**, inset from the window with a hairline edge, two sheets showing behind it and a hard shadow. The sheets behind are real rectangles rather than a wide shadow spread: a stack has *edges*, and an edge is what says there is more underneath rather than that this is floating. The canvas is mixed a shade deeper than the palette's own, because a sheet on a background its own colour is not a sheet, it is a margin. |
