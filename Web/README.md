...
| WV-11 | `icon.svg` is **generated** by `../macOS/Scripts/make-icons.swift` from `Packaging/Logo.svg`, with the same plate geometry and the same measured placement as the Mac and iOS icons. Checked by rendering it: the plate is 0.828 of the canvas, the mark's circle 0.662 of the plate, and the dark ring's balance point lands within half a pixel of where the `.icns` puts it. It is self-contained rather than referencing the logo, because a browser renders an SVG favicon in a restricted mode that fetches nothing external. |
